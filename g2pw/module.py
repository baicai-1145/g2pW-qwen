...
# Import our Qwen3 adapter
try:
    from .qwen3_adapter import Qwen3ForG2PW, Qwen3ConfigForG2PW, create_qwen3_config_from_pretrained
except ImportError:
...
class G2PW(BertPreTrainedModel):
    def __init__(self, model_source, labels, chars, pos_tags,
                 ...
                 use_qwen3=False, qwen3_model_path=None):

        # Initialize with appropriate config
        if use_qwen3 and qwen3_model_path:
            # For Qwen3, we need to handle config differently
            self.use_qwen3 = True
            self.qwen3_config = create_qwen3_config_from_pretrained(qwen3_model_path)
            # Create a dummy BERT config for compatibility
            from transformers import BertConfig
            dummy_config = BertConfig(hidden_size=self.qwen3_config.hidden_size)
            super().__init__(dummy_config)
            self.config = self.qwen3_config  # Override with Qwen3 config
        else:
        ...
        if self.use_qwen3:
            self.backbone = Qwen3ForG2PW(self.qwen3_config)
            print(f"Using Qwen3 backbone with hidden_size={self.config.hidden_size}")
        else:
            self.backbone = BertModel(self.config)
            print(f"Using BERT backbone with hidden_size={self.config.hidden_size}")
        ...
        self.param_pos = param_pos or {}  # Provide default empty dict
    ...
    @classmethod
    def create_qwen3_model(cls, qwen3_model_path, labels, chars, pos_tags,
                          use_conditional=False, param_conditional=None,
                          use_focal=False, param_focal=None,
                          use_pos=False, param_pos=None,
                          load_pretrained=True, freeze_backbone=False):
        """
        Create a G2PW model with Qwen3 backbone and optional pretrained weights.

        Args:
            qwen3_model_path: Path to the Qwen3 model directory
            labels: List of phoneme labels
            chars: List of characters
            pos_tags: List of POS tags
            use_conditional: Whether to use conditional mechanism
            param_conditional: Parameters for conditional mechanism
            use_focal: Whether to use focal loss
            param_focal: Parameters for focal loss
            use_pos: Whether to use POS tagging
            param_pos: Parameters for POS tagging
            load_pretrained: Whether to load pretrained weights
            freeze_backbone: Whether to freeze backbone parameters

        Returns:
            G2PW model instance with Qwen3 backbone
        """
        # Create model instance
        model = cls(
            model_source=None,  # Not used for Qwen3
            labels=labels,
            chars=chars,
            pos_tags=pos_tags,
        ...
        if load_pretrained and qwen3_model_path:
            try:
                # Try different import methods
                try:
                    from .pretrained_loader import load_pretrained_weights_for_g2pw
                except ImportError:
                    from pretrained_loader import load_pretrained_weights_for_g2pw

                success = load_pretrained_weights_for_g2pw(
                    model,
                    qwen3_model_path,
                ...
                if success:
                    ...
                    print(f"⚠️ Pretrained weights loading failed, using random initialization")
            except Exception as e:
                print(f"⚠️ Pretrained weights loading error: {e}")
    ...
    def _get_char_pos_ids(self, char_ids, pos_ids):
        if pos_ids is None:
            # If POS is disabled, use zero pos_ids
            pos_ids = torch.zeros_like(char_ids)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, phoneme_mask, char_ids, position_ids, pos_ids=None, label_ids=None, eps=1e-6):
        # Forward through backbone model (BERT or Qwen3)
        if self.use_qwen3:
            # Qwen3 doesn't use token_type_ids, so we ignore it
            sequence_output, pooled_output = self.backbone(
                input_ids=input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,  # Will be ignored by Qwen3ForG2PW
                return_dict=False
            )
        else:
            # Original BERT logic
        ...
        if self.use_conditional:
            if (self.param_conditional['char+pos-second']
                ...
                pred_pos_ids = pos_ids if self.training or not self.param_pos.get('pos_joint_training', False) \
